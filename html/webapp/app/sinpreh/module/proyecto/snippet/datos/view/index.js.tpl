...
<script>
    var snippet_general_form = function(){
        ...
        var convenio_programa_opt = $("#programa_convenio_id");
        ...
        */
        ...
        var handle_option_convenio_programa = function(){
            /*
            $('#programa_id').on('change',function(){
            var id = $('#programa_id').val();
                convenio_programa_search(id);
            });
        ...
        var convenio_programa_search = function(id){
            convenio_programa_opt.find("option").remove();
            if(id!="") {
                $.post(urlmodule+"/get.convenio_programa"
                    , {id: id}
                    , function (res, textStatus, jqXHR) {
                        let selOption = $('<option></option>');
                        convenio_programa_opt.append(selOption.attr("value", "").text("{/literal}{#field_holder_programa_convenio_id#}{literal}"));
                        let convenio_programa_list = []
                        for (var row in res) {
                            convenio_programa_opt.append($('<option></option>').attr("value", res[row].id).text(res[row].codigo));
                            convenio_programa_list[res[row].id] = res[row];
                        }
                        convenio_programa_opt.trigger('chosen:updated');
                    }
                    , 'json');
                convenio_programa_opt.prop('disabled', false);
            }else{
                //handle_options_init();
            }
        };
        ...
        return {
            init: function() {
                ...
                handle_option_convenio_programa();
            }
        };
    }();
